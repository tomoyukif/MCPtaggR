...
# Make SNP list from SyRI results
#' Import the SyRI output files
#' 
#' Import .out file generated by SyRI and then apply a filtering 
#' to only retain SNPs in collinear blocks detected between the reference and
#' the alternative genomes given to SyRI
#' 
#' @param syri_fn A path to a .out file generated by SyRI
#'
#' @importFrom data.table fread
#' @importClassesFrom IRanges IRanges
#' @importClassesFrom GenomicRanges GRanges
...
#' @export
#'
syri2SNPs <- function(syri_fn){
    snps <- fread(file = syri_fn,
                  header = FALSE,
                  sep = "\t",
                  colClasses = c("factor", "character", "character",
                                 ...
                                 "factor", "factor"))
    v10 <- gsub("[0-9]", "", snps$V10)
    snps <- subset(snps, subset = v10 %in% c("INV", "SYN"))
    snps <- subset(snps, subset = V11 %in% "SNP")
    snps <- subset(snps, select = c(V1:V2, V4:V7, V9:V10))
    snps$V1 <- droplevels(snps$V1)
    snps$V4 <- droplevels(snps$V4)
    snps$V5 <- droplevels(snps$V5)
    snps$V6 <- droplevels(snps$V6)
    snps$V9 <- droplevels(snps$V9)
    snps$V10 <- droplevels(snps$V10)
    
    message("Number of SNPs: ", nrow(snps))
    ref_chr <- levels(snps$V1)
    ref_chr <- gsub("[^0-9]", "", ref_chr)
    ref_chr <- as.numeric(ref_chr)
    ref_chr <- ref_chr[as.numeric(snps$V1)]
    alt_chr <- levels(snps$V6)
    alt_chr <- gsub("[^0-9]", "", alt_chr)
    alt_chr <- as.numeric(alt_chr)
    alt_chr <- alt_chr[as.numeric(snps$V6)]
    
    ref_snps <- GRanges(seqnames = snps$V1, 
                        ranges = IRanges(start = as.numeric(snps$V2),
                                         width = 1), 
                        allele = snps$V4,
                        id = snps$V9,
                        Synteny = snps$V10)
    alt_snps <- GRanges(seqnames = snps$V6, 
                        ranges = IRanges(start = as.numeric(snps$V7),
                                         width = 1), 
                        allele = snps$V5,
                        id = snps$V9,
                        Synteny = snps$V10)
...
digestGenome <- function(snps,
    ...
    if(check){
        ref_reads <- .getReads(genome = ref_genome, seg = ref_seg, read_len = read_len)
        writeXStringSet(ref_reads$read1, fastq_fn[1],
                        compress = TRUE, format = "fastq")
        writeXStringSet(ref_reads$read2, fastq_fn[2],
                        compress = TRUE, format = "fastq")
        
        alt_reads <- .getReads(genome = alt_genome, seg = alt_seg, read_len = read_len)
