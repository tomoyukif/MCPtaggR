...
#' 
#' @param snps A SNPs object.
#' @param ref_fn A path to a reference genome FASTA. 
#' @param alt_fn A path to an alternative genome FASTA.
#' @param out_dir A path to the output directory.
#' @param out_fn A prefix of the output file names.
#' @param ref_pat A string to specify a pattern to subset the reference genome data.
#' @param alt_pat A string to specify a pattern to subset the alternative genome data.
#' @param read_len The maximum length of reads in FASTQ to be mapped on the 
#' genomes in the read mapping step.
#' @param re The recognition sequences of restriction enzymes used in your 
#' reduced representation sequencing. 
...
digestGenome <- function(snps,
    ...
    ref_genome <- readDNAStringSet(ref_fn)
    alt_genome <- readDNAStringSet(alt_fn)
    ref_genome <- grepl(ref_pat, ref_genome)
    alt_genome <- grepl(alt_pat, alt_genome)
    if(length(ref_genome) != length(alt_genome)){
        stop("The number of chromosomes does not match between the reference",
             ...
             "\n specify a pattarn to subset the data to `ref_pat` and `alt_pat`.")
    }
    ...
    fastq_fn <- c(paste0(out_dir, "/", out_fn, "_ref_R1.fq.gz"),
                  paste0(out_dir, "/", out_fn, "_ref_R2.fq.gz"),
                  paste0(out_dir, "/", out_fn, "_alt_R1.fq.gz"),
                  paste0(out_dir, "/", out_fn, "_alt_R2.fq.gz"))
    names(fastq_fn) <- c("ref_r1", "ref_r2", "alt_r1", "alt_r2")
    genome_fn <- paste0(out_dir, "/", out_fn, "_merge.fa.gz")
    check <- TRUE
    if(reuse){
        if(all(file.exists(fastq_fn)) & file.exists(genome_fn)){
            check <- FALSE
...
alignTAG <- function(dg,
    ...
    bam_fn <- paste0(sub("_R1\\.fq\\.gz", "", dg$fq[c(1, 3)]), "_aln.bam")
